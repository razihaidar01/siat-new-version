...
const CertificateManager = () => {
  ...
  const downloadQR = async (certNum: string) => {
    const verifyUrl = getVerifyUrl(certNum);
    ...
    <div className="space-y-8">
      ...
      {createdCert && (
        <div className="glass-card p-6 rounded-xl border-2 border-green-500/30">
          <h3 className="text-lg font-display font-bold text-green-600 mb-2">✓ Certificate Created: {createdCert}</h3>
          ...
          <div className="flex gap-3 flex-wrap">
            <button onClick={() => downloadQR(createdCert)}
              className="btn-primary-glow !py-2.5 !px-5 flex items-center gap-2">
